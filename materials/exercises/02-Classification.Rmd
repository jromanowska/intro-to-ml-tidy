...
so_all_tmp <- bind_rows(
  so_train %>% mutate(dataset = "train"),
  so_test %>% mutate(dataset = "test")
)
ggplot(so_all_tmp, aes(remote, years_coded_job)) +
  geom_boxplot() +
...
tree_fit <- fit_split(formula = remote ~ years_coded_job + salary, 
                      model = tree_spec, 
                      split = so_split)
tree_fit %>%   
  collect_metrics()
...
tree_fit
tree_fit %>% tidyr::unnest(cols = .predictions)
```
...
# or using the confusion matrix function:
tree_fit %>%   
  collect_predictions() %>% 
  conf_mat(truth = remote, estimate = .pred_class)

# to show specificity and selectivity:
fit_split(remote ~ years_coded_job + salary, 
          model = tree_spec, 
          split = so_split,
          metrics = metric_set(accuracy, sens, spec)) %>% # here!
  collect_metrics()
```
